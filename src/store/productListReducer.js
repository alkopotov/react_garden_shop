...
export const productListReducer = (state = defaultState, action) => {
  switch(action.type) {
    case GET_ALL_PRODUCTS:
      return {title: 'All products', data: action.payload};
    case GET_PRODUCTS_BY_CATEGORY:
      return {title: action.payload.category.title, data: action.payload.data};
    case GET_ALL_PRODUCTS_ON_SALE:
      return {title: 'Discounted items', data: action.payload}
    case GET_TOP_PRODUCTS_ON_SALE:
      return {title: 'Sale', data: action.payload.sort((a,b) => a.discont_price / a.price - b.discont_price / b.price).slice(0,4)}
    default:
      return state
  }
}
